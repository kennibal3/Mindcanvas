...
3. label 字段为短词组（≤18 字），不得含换行符，不得为空。
4. id 字段由字母数字下划线组成，全局唯一，不得重复。
5. 内容必须忠于原文，不编造。

结构约束（REQ-050，违反会导致节点在画布上丢失，务必逐条自查）：
6. 有且只有一个 parent 为 "" 的根节点，不允许出现第二个。
7. 除根节点外，每个节点的 parent 必须精确等于本次输出中已存在的某个节点 id；
   不得指向不存在的 id，不得指向自己，不得让 parent 关系绕成闭环。
8. edges 中每条连线的 from 与 to 也必须是本次输出中存在的节点 id。
9. 宁可少给节点，也不要给出挂不上主干的游离节点。
10. 输出前请自查：把每个节点沿 parent 一路向上，都必须能走到那个唯一的根节点。
`
...
流程图硬约束（REQ-050，违反会画出断头流程或分不清走向的分支）：
- 有且只有一个 node_type="start"，且它就是那个 parent="" 的根节点
- 至少有一个 node_type="end"；每条分支最终都要走到某个 end，不能半路断掉
- 每个 node_type="decision" 必须有至少两条出边（写在 edges 里），
  且这些出边的 label 必须都非空、互不相同（如"是"/"否"、"达标"/"不达标"）
- 如果原文里某个判断只交代了一种结果，另一种请据原文语义补成明确的收束步骤，
  不要留下只有一条出路的判断节点
` + diagramCommonConstraints
